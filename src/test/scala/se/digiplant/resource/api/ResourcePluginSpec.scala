...
object ResourcePluginSpec extends Specification {
  ...
  "Resource Plugin" should {
    ...
    "put a resource into a supplied source" in new FakeApp {
    ...
    "put a resource with metadata" in new FakeApp {
    ...
    "get a resource from a supplied source" in new FakeApp {
      val fileuid = res.put(getTestFile, "images")
    ...
    "get a resource with the supplied metadata" in new FakeApp {
      val fileuid = res.put(getTestFile, meta = Seq("100", "100", "auto"))
      val file = res.get(fileuid.get)
    ...
    "delete a resource" in new FakeApp {
      val fileuid = res.put(getTestFile)
    ...
    "delete a resource with supplied metadata" in new FakeApp {
      val fileuid = res.put(getTestFile, meta = Seq("100", "100", "auto"))
